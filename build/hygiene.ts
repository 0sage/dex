...
export function checkCopilotEnginesVersion(repoRoot: string): string | undefined {
	...
	if (!fs.existsSync(copilotPkgPath)) {
		// The copilot extension is not present in this build; nothing to check.
		return undefined;
	}
...
export function hygiene(some: NodeJS.ReadWriteStream | string[] | undefined, runEslint = true): NodeJS.ReadWriteStream {
	...
	// Upstream checked product.json here and rejected any 'extensionsGallery':
	// that config points at Microsoft's Marketplace, which is not licensed for
	// Code - OSS builds. Dex configures Open VSX instead, which is
	// vendor-neutral, so the key is expected in this fork and the check is gone.
